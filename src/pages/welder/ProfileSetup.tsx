...
export const WelderProfileSetup: React.FC = () => {
  const { user } = useAuth();
  const navigate = useNavigate();
  const [currentStep, setCurrentStep] = useState(1);
  const [loading, setLoading] = useState(false);

  const [profile, setProfile] = useState<WelderProfile>({
    userId: user?.id || '',
    personal: {
      firstName: '',
      lastName: '',
  ...
  useEffect(() => {
    if (user) {
      const existingProfile = storageUtils.getWelderProfile(user.id);
      if (existingProfile) {
  ...
  return (
    <Layout title="Welder Profile Setup">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="flex justify-between mt-8">
              <Button
                variant="outline"
                onClick={() => currentStep > 1 ? setCurrentStep(currentStep - 1) : navigate('/welder/dashboard')}
              >
