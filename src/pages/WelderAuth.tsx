...
import { Wrench, ArrowLeft, Sun, Moon } from "lucide-react";
import { Link } from "react-router-dom";
import { toast } from "sonner";
import gsap from "gsap";
import { useTheme } from "@/contexts/ThemeContext";

export const WelderAuth: React.FC = () => {
  ...
  const { login, registerWelder, isAuthenticated } = useAuth();
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const success = await registerWelder(
        registerData.email,
    ...
    <div className="min-h-screen flex items-center justify-center p-4 ">
      <Card className="auth-card w-full max-w-md relative overflow-hidden">
        ...
        <CardHeader className="text-center space-y-2">
          ...
          <CardTitle className="text-2xl font-bold">Welder Portal</CardTitle>
          <CardDescription>
            Login to your account or register as a new welder
          </CardDescription>
        </CardHeader>
